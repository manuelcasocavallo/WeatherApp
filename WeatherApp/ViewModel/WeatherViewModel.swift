...
class WeatherViewModel: ObservableObject {
    ...
    func updateWeather(city: String) {
        ...
        guard let url = URL(string: baseURL + cityName + unit + apiKey) else { return }
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, _, error) in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    print("Success\nCity: \(self.currentData.name)\nTemp: \(self.currentData.temp)\nFeels like: \(self.currentData.feelsLike)\nImage name: \(self.currentData.conditionsImageName)")
                    print(url)
                }
            } catch {
